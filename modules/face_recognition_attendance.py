...
class Session:
    ...
    def make_known_entry(self, student_id: str, student_university: str, date, time):
        self.__cursor.execute(f"SELECT entry_exit FROM {self.__mysql_attendance_table} WHERE student_id = %s AND university = %s AND date = %s", (student_id, student_university, date))
        entry_exit = self.__cursor.fetchone()[0]

        if entry_exit is None:
            self.__cursor.execute("SET SQL_SAFE_UPDATES = 0;")
            self.__cursor.execute(
                f"UPDATE {self.__mysql_attendance_table} SET attendance = 0, in_time = %s, entry_exit = %s WHERE student_id = %s AND university = %s AND date = %s",
            ...
            if self.checking_time_interval(self.__time_between_entries, last_entry): # Only execute if desired time interval in seconds have passed between last entry and now
                ...
                self.__cursor.execute("SET SQL_SAFE_UPDATES = 0;")
                self.__cursor.execute(
                    f"UPDATE {self.__mysql_attendance_table} SET attendance = attendance + 1, entry_exit = %s WHERE student_id = %s AND university = %s AND date = %s",
    ...
    def start_session(self, camera_index=0, show_preview=True, scale_frame=0.77, desired_fps=2, tolerance=0.45):
        try:
            ...
            while True:
                ...
                rgb_frame = cv2.cvtColor(small_frame, cv2.COLOR_BGR2RGB) # Convert the frame from BGR to RGB
                if rgb_frame.dtype != 'uint8':
                    continue  # Skip the frame if it's not in the correct format
